...
BetterSoftwareSerial::BetterSoftwareSerial(uint8_t rxPin, uint8_t txPin, bool inverse_logic /* = false */)
{
...
void BetterSoftwareSerial::begin(unsigned long speed)
{
	if (port) {
		port->begin(speed);
	} else {
...
size_t BetterSoftwareSerial::write(uint8_t b)
{
...
void BetterSoftwareSerial::flush()
{
	if (port) port->flush();
}
...
int BetterSoftwareSerial::available()
{
	if (port) return port->available();
	return 0;
}

int BetterSoftwareSerial::peek()
{
	if (port) return port->peek();
	return -1;
}

int BetterSoftwareSerial::read()
{
...
// Statics
//
BetterSoftwareSerial *BetterSoftwareSerial::active_object = 0;
char BetterSoftwareSerial::_receive_buffer[_SS_MAX_RX_BUFF]; 
volatile uint8_t BetterSoftwareSerial::_receive_buffer_tail = 0;
volatile uint8_t BetterSoftwareSerial::_receive_buffer_head = 0;
...
/* static */ 
inline void BetterSoftwareSerial::tunedDelay(uint16_t delay) { 
  uint8_t tmp=0;
...
bool BetterSoftwareSerial::listen()
{
...
//
void BetterSoftwareSerial::recv()
{
...
void BetterSoftwareSerial::tx_pin_write(uint8_t pin_state)
{
  if (pin_state == LOW)
...
uint8_t BetterSoftwareSerial::rx_pin_read()
{
...
/* static */
inline void BetterSoftwareSerial::handle_interrupt()
{
  if (active_object)
  {
    active_object->recv();
...
ISR(PCINT0_vect)
{
  BetterSoftwareSerial::handle_interrupt();
}
#endif
...
// Constructor
//
BetterSoftwareSerial::BetterSoftwareSerial(uint8_t receivePin, uint8_t transmitPin, bool inverse_logic /* = false */) : 
...
// Destructor
//
BetterSoftwareSerial::~BetterSoftwareSerial()
{
  end();
}

void BetterSoftwareSerial::setTX(uint8_t tx)
{
  pinMode(tx, OUTPUT);
...
void BetterSoftwareSerial::setRX(uint8_t rx)
{
  pinMode(rx, INPUT);
  if (!_inverse_logic)
...
void BetterSoftwareSerial::begin(long speed)
{
